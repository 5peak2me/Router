package io.github.speak2me.router.internal
...
internal object FastServiceLoader {
    ...
    /**
     * This method attempts to load [RouteTable] in Android-friendly way.
     *
     ...
     *
     * We also use SL for [RouteTable], but we do not experience the same problems and CEH is a public API
     * that may already be injected vis SL, so we are not using the same technique for it.
     */
    @JvmStatic
    fun loadRouteTable(): List<RouteTable> {
        val clz = RouteTable::class.java
        if (!ANDROID_DETECTED) {
         ...
         */
        return try {
            val result = ArrayList<RouteTable>(2)
            val mainFactory = createInstanceOf(clz, "com.chenenyu.router.template.RouteTable")
            if (mainFactory == null) {
